...
| leftover `x` or `y` + one-sided local order | `Problem_Factor` | `leftover_mismatch_factors`, `leftover_x_mismatch_factors_pin`, `order_mismatch_factors_pin`, `leftover_77_mismatch_factors` | leftover always factors; residual leaf as factoring; RSA ≡ or ≢ factoring; PPT |
...
| residual leaf + mismatch | `Problem_Factor` (leaf unused; mismatch is load-bearing) | `residual_mismatch_factors`, `residual_mismatch_factors_pin` | `srsa_residual_leaf` as `Problem_Factor` on its own |
