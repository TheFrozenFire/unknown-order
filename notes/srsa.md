...
Dixon / QS combination (`dixon_pin_splits`, CAS `161`) and NFS two-sided
...
covers it, not because they inverted `y`. Numbers from `Pin.v`. Setup identity
`hom_quad_remainder` (CAS `162`). Notes: `notes/sieve-rsa.md`.
...
is settled. Two writings of an *e*-th root polynomial on units
(CAS `164`, `SrsaRootPoly.v`): the CRT binomial
`c_p X^{d_p}+c_q X^{d_q}` inverts every unit and
`gcd(c_p,N)`, `gcd(c_q,N)` are the factors; the monomial `X^d`
agrees as a function on units, its coefficients do not split, and
...
CAS `233`). A residual solver that always returns one
residual-shaped `e`, given a known inverse of that `e` modulo `λ`,
is the trapdoor map `y ↦ y^{e^{-1}}`; Miller-from-`e d'−1` splits
(`residual_solver_reduced_fixed_e_constructs_factor`, inhabitant
at `e=7`, CAS `234`–`236`). Miller uses the inverse, not the
solver. Not
...
| varying `e` / `λ+1` output gcds | `residual_solver_reduced_e_cong_*` / `pin_miller_from_lambda_multiple` | `SrsaVaryingE.v` | `228`–`236` |
...
| Dirichlet assoc `{id,f,f}` and order-3 pin `(f∘f)∘f = f∘(f∘f)` on `Cl(−31)` | `compose_assoc_id_ff` / `compose_assoc_neg31_ord3` | `BinForms.v`, `ClassGroupWall.v` | `251` |
