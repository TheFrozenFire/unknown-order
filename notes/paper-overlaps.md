...
| 8 | “RSA ≡ factoring” in the standard model, or cites Aggarwal–Maurer / generic ring as discharging that | Model mismatch | `Refuse_RSA_eq_factoring_standard_model`; `Refuse_AM09_generic_ring_as_standard_model` | AM09 is generic-ring: `gra_inv_nonunit_factors`, `gra_nodiv_identical_root_impossible_X3`, `gra_const_lambda_plus_one_solves_sRSA_without_factoring`. Boneh–Venkatesan unwind is `bv_few_query_low_e_drops_oracle` (not “RSA is easier”). Jacobi shows GRA-hard ⇏ standard-hard (`jacobi_two_values`). Coron–May / Miller-from-`(e,d)` is *not* the converse. This repo will not treat Factoring ≤ RSA as a design target. |
...
| 10 | Rabin / RW inversion oracle on a planted square, or treats that as RSA inversion ⇒ factor | Mixed `√1` / non-associate root | `rabin_oracle_nonassociate_factors`; `rabin_roots_split` | A root of `r²` that is not `±r` splits `N`. An RSA `e`-th-root inverter does **not** (`rsa_inverter_constructs_factor_named`, unused; row 8). |
