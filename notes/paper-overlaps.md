...
Companion to `notes/hardness.md` (relations),
`generated/NAMED_SKIPS.md` (refuses), `Hardness.v` / `Presentation.v`
(winning conditions), and `notes/arxiv-corpus.md`.
...
| 8 | “RSA ≡ factoring” in the standard model *by citing* Aggarwal–Maurer / generic ring | Model mismatch | `Refuse_AM09_generic_ring_as_standard_model`; live `rsa_inverter_constructs_factor_open_named`, `strong_rsa_solver_constructs_factor_open_named` | AM09 is generic-ring: `gra_inv_nonunit_factors`, `gra_nodiv_identical_root_impossible_X3`, `gra_const_lambda_plus_one_solves_sRSA_without_factoring`. Boneh–Venkatesan unwind is `bv_few_query_low_e_drops_oracle`. Jacobi shows GRA-hard ⇏ standard-hard (`jacobi_two_values`). Coron–May / Miller-from-`(e,d)` is knowledge of `(e,d)`, not an inverter-only reduction. Those are inroads in restricted models. The standard-model solver ⇒ factor implication is an open Gallina target, not a refuse. |
| 9 | LLX-style non-membership, and `μ = ∏_{s∈S} s (mod φ(N))` (or `λ`) is given to the adversary | Peng–Bao 2010 | `llx_lambda_forges_nonmem`; `peng_bao_member_still_forges` | Completeness is Bézout (`llx_complete`). If `gcd(x, 1−aθ)=1`, Shamir extracts an `x`-th root of `g` (`llx_extract_root`). Knowing `θ` and `λ` forges a witness even when `x` divides `θ`. 2024/505 is out of scope of the attack only if `μ` is never published. CAS `42`. |
| 10 | Rabin / RW inversion oracle on a planted square, or treats that as RSA inversion ⇒ factor | Mixed `√1` / non-associate root | `rabin_oracle_nonassociate_factors`; `rabin_roots_split` | A root of `r²` that is not `±r` splits `N`. An RSA `e`-th-root inverter is the live converse (`rsa_inverter_constructs_factor_open_named`, unproved; row 8). |
| 11 | Decrypt/sign implementation that checks a predicate on one CRT side, or emits a CRT-faulty signature | One-sided / Bellcore | `one_sided_congruence_factors`; `bellcore_factors`; `onesided_plain_one_factors` | A check that is not a function of recombined `m` is Type B’s winning condition. Constructor `m_p ≡ 1 (mod r)` need not factor (`ctor_slot_mod_r_need_not_factor`). |
...
- the row number;
- the Rocq name, or a `NamedRefuse` / `*_named` if the row is a refuse;
