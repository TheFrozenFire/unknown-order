...
| Leander–Rupp ASIACRYPT 2006 | GRA, no division, low `e` | `gra_nodiv_identical_root_impossible_X3`; `gra_eq_leak_factors`; identity `X^3−X` does not vanish on pin units (`residual_X3_roots_mod_11`, `residual_low_degree_identity_not_all_Fp_units`); `GConst` linear `−1` (`residual_const_N_ndiv_linear`) |
