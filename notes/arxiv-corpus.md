...
| Leander–Rupp ASIACRYPT 2006 | GRA, no division, low `e` | `gra_nodiv_identical_root_impossible_X3`; `gra_eq_leak_factors`; identity `X^3−X` does not vanish on pin units (`residual_X3_roots_mod_11`, `residual_low_degree_identity_not_all_Fp_units`); `GConst` linear `−1` (`residual_const_N_ndiv_linear`); any low-degree nodiv tape vanishing on `1..10` has `11` dividing every coeff (`residual_nodiv_low_degree_units_divides_11`); vanishing on `(Z/NZ)*` CRT-lifts to `N` dividing every coeff (`residual_nodiv_low_degree_ZN_units_divides_N`); a nodiv handle of `gra_deg_bound ≤ 3` is inside that window (`residual_nodiv_short_ZN_units_divides_N`); exact `deg(PQ)=deg P+deg Q` (`poly_degree_mul`); square/cube tapes miss units (`residual_square_cannot_vanish_on_ZN_units`, `residual_cube_cannot_vanish_on_ZN_units`); trapdoor `X^{27}` inverts the pin but is not low-degree (`residual_trapdoor_inverts_pin`) |
...
## B. Named cells that are now theorems (or named refuses)

| Paper | Cell | Rocq |
|---|---|---|
| Dobson–Galbraith–Smith [arXiv:2211.16128](https://arxiv.org/abs/2211.16128) | `h(Δ)` trivializes AR search on `Cl`, same as `λ+1` on units | `annihilator_plus_one_is_strong_RSA`; `class_number_solves_AR_neg31`. `D+1` is not `h`. Sutherland cost stays named. |
| Bünz–Fisch–Szepieniec DARK | order assumption; `r`-fractional root | `Problem_Annihilator`; `Problem_FractionalRoot`. RSA/sRSA are fractional root at `b=1`. |
| Hhan [arXiv:2402.11269](https://arxiv.org/abs/2402.11269); Damgård–Koprowski 2002 | UO generic-group lower bounds | Interpreter `ggm_eval`. Query lower bounds `Refuse_UO_GGM`. Not standard-model. |
| Jurkiewicz [arXiv:2503.00950](https://arxiv.org/abs/2503.00950) | even-order / `√1` factors `N` | Overlap only: `mixed_sqrt1_splits`, `rabin_roots_split`. |
